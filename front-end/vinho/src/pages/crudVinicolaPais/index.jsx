...
export default function CrudVinicolaPais() {
  ...
  const [buscaVinicola, setBuscaVinicola] = useState("");
  ...
  async function BuscarVinicola() {
    if (!buscaVinicola || buscaVinicola.trim().length < 1) {
        alert("O campo de busca deve estar preenchido");
        return;
    }

    if (!isNaN(buscaVinicola) && Number.isInteger(Number(buscaVinicola.trim()))) {
      BuscarVinicolaPorId();
    } else {
      BuscarVinicolaPorNomeVinicola();
    }
  }

  async function BuscarVinicolaPorNomeVinicola() {
    try {
      const resp = await axios.get(
        `http://localhost:5001/vinicola/nome/${buscaVinicola}`
      );
      ...
      alert(
        `Vinícola com nome "${vinicolaBuscado.vinicola}" buscado com sucesso!`
      );
    } catch (error) {
      alert(
        error.response?.data?.erro ??
          `Erro ao Buscar a vinícola ${vinicola.vinicola}`
      );
    }
  }

  async function BuscarVinicolaPorId() {
    try {
      const resp = await axios.get(
        `http://localhost:5001/vinicola/${buscaVinicola}`
      );
      const vinicolaBuscado = resp.data;

      setVinicola(vinicolaBuscado);

      alert(`Vinícola com id "${buscaVinicola}" buscado com sucesso!`);
    } catch (error) {
      alert(
        error.response?.data?.erro ??
          `Erro ao Buscar a vinícola de id ${buscaVinicola}`
      );
    }
  }

  async function BuscarPais() {
    if (!buscaPais || buscaPais.trim().length < 1) {
        alert("O campo de busca deve estar preenchido");
        return;
    }
    
    if (!isNaN(buscaPais) && Number.isInteger(Number(buscaPais.trim()))) {
      BuscarPaisPorId();
    } else {
      BuscarPaisPorNomePais();
    }
  }

  async function BuscarPaisPorNomePais() {
    try {
      const resp = await axios.get(
        `http://localhost:5001/pais/nome/${buscaPais}`
      );
      const paisBuscado = resp.data[0];

      setPais(paisBuscado);

      alert(`Pais com nome "${paisBuscado.pais}" buscado com sucesso!`);
    } catch (error) {
      alert(error.response?.data?.erro ?? `Erro ao Buscar o pais ${pais.pais}`);
    }
  }

  async function BuscarPaisPorId() {
    try {
      const resp = await axios.get(`http://localhost:5001/pais/${buscaPais}`);
      const paisBuscado = resp.data;

      setPais(paisBuscado);

      alert(`Pais de id "${buscaPais}" buscado com sucesso!`);
    } catch (error) {
      alert(
        error.response?.data?.erro ?? `Erro ao Buscar o pais de id ${buscaPais}`
      );
    }
  }
  ...
  async function cadastrarVinicola() {
    try {
      ...
      const resp = await axios.post(url, vinicola);
  ...
  async function excluirVinicola() {
    try {
      await axios.delete(
        `http://localhost:5001/vinicola/${vinicola.id_vinicola}`
      );
  ...
  const listarVinicola = async () => {
    try {
      ...
      alert(
        error.response?.data?.erro ??
          "Erro ao buscar as informações das vinícolas."
      );
    }
  };
  ...
  async function cadastrarPais() {
    try {
      ...
      const resp = await axios.post(url, pais);
  ...
  const listarPais = async () => {
    try {
      ...
      alert(
        error.response?.data?.erro ??
          "Erro ao buscar as informações dos países."
      );
    }
  };
      ...
      <TelaCarregamento tempo={250}>
        ...
        <section className="banner-abas">
          <div className="titulo-banner">
            <h1>Manipulação das Vinícolas/Paises</h1>
          </div>
              ...
              <div className="container-conteudo">
                <div className="pesquisa">
                  <input
                    type="button"
                    value="Buscar"
                    onClick={BuscarVinicola}
                  />
                  <input
                    type="text"
                    value={buscaVinicola}
                    onChange={(e) => setBuscaVinicola(e.target.value)}
                ...
                <div className="botoes-crud">
                  <input
                    type="button"
                    value="Cadastrar"
                    onClick={cadastrarVinicola}
                  />
                  <input
                    type="button"
                    value="Alterar"
                    onClick={alterarVinicola}
                  />
                  <input
                    type="button"
                    value="Excluir"
                    onClick={excluirVinicola}
                  />
                  <input
                    type="button"
                    value="Limpar"
                    onClick={limparCamposVinicola}
                  />
                </div>
              </div>
            </div>
                ...
                <div className="dados">
                  <div>
                    ...
                    <input
                      ...
                      value={pais.id_pais}
                      onChange={(e) =>
                        setPais({ ...pais, id_pais: e.target.value })
                      }
                ...
                <div className="dados">
                  <div>
                    ...
                    <input
                      ...
                      value={pais.pais}
                      onChange={(e) =>
                        setPais({ ...pais, pais: e.target.value })
                      }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={pais.sigla}
                      onChange={(e) =>
                        setPais({ ...pais, sigla: e.target.value })
                      }
