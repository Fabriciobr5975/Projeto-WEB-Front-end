...
export default function Produtos() {
  ...
  const indiceInicioExibicao =
    vinhosFiltrados.length === 0 ? 0 : (paginaAtual - 1) * itensPorPagina + 1;
  const indiceFimExibicao = Math.min(
    paginaAtual * itensPorPagina,
    vinhosFiltrados.length
  );
  ...
  function handleFiltro(tipo, valor, checked) {
    setFiltros((prevFiltros) => {
      ...
      if (checked) {
        ...
        novosFiltros[tipo] = novosFiltros[tipo].filter(
          (item) => item !== valor
        );
      }
      setPaginaAtual(1);
      ...
      <TelaCarregamento tempo={250}>
        ...
        <section className="titulo-banner">
          ...
          <p className="paginas">
            ...
            Mostrando {indiceInicioExibicao} - {indiceFimExibicao} de{" "}
            {vinhosFiltrados.length} resultados
          </p>
        </section>
